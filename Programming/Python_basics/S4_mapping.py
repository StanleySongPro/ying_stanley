...
class Mapping:
    ...
    def in_payrcv(self, inputStr=None):
        if "Credit" in inputStr:
            return "R"
        elif "Debit" in inputStr:
            return "P"
    ...
    # in_event_date
    def get_bzday(self, inputDate=None, save_format="%d-%b-%y"):
        """
        寻找inputDate前一个工作日
        """
        PH_dateobj = []
    ...
    def processing(self, input_file=None, output_file=None, tmpt_file=None):
        ...
        df_out["in_pb_account"] = (
            df_out["#in_principal"] + "_" + df_out["in_prime_broker"] + "_ACCT_ID"
        )
        ...
        # in_amt
        df_out["in_amt"] = df_in["Transaction Amount"]
        ...
        df_out.dropna(subset=["in_event_type"], inplace=True)
...
#    timestamp = previous_workday.strftime('%Y%m%d')
    ...
    input_file_list = [os.path.join(input_folder, filename) for filename in os.listdir(input_folder)]
    output_file_list = [os.path.join(output_folder, filename.split('.')[0]+'_out.csv') for filename in os.listdir(input_folder)]
    
#    input_file = os.path.join(BASE, "DailyBAL_222510173.xls")
#    output_file = os.path.join(BASE,"DailyBAL_out.csv")
